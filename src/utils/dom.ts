 ...
 *   track elements created by `useSEO`. When provided, any duplicate
 *   element removed by the dedup loop is also `delete()`d from the Set so
 *   detached references don't linger and prevent garbage collection.
 *   Backwards-compatible: omit to keep the previous behavior (DOM-only
 *   removal).
 ...
 * @param trackedElements - Optional `Set<Element>` that the caller uses to
 *   track elements created by `useSEO`. When provided AND the unique
 *   constraint removes duplicate elements, those duplicates are also
 *   `delete()`d from the Set so detached references don't linger and
 *   prevent garbage collection. Backwards-compatible: omit to keep the
 *   previous behavior (DOM-only removal).
